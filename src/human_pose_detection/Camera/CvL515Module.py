...
class CvL515Module():
    def __init__(self, filepath):
        ...
        self.color_intrinsic_path = self.package_path + "/" + str("color_intrinsic_l515") + ".npy"
        self.depth_intrinsic_path = self.package_path + "/" + str("depth_intrinsic_l515") + ".npy"

        self.setCameraParams()

    def setCameraParams(self):
        self.color_intrinsic = self.setup_intrinsic(self.color_intrinsic_path)
        self.depth_intrinsic = self.setup_intrinsic(self.depth_intrinsic_path)

    def getImages(self):
        
        if(os.path.isfile(self.package_path + "/" +  self.data_path) == True):
            # print("single file")
